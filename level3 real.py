...
# PATH OPTIMISATION     # 
#########################   
    tasks={'A':0,'B':0,'C':0,'D':0} 
    for line in urllib2.urlopen("http://tiny.cc/ptqrdx"):
...
############################  
    ...
    def getNextValues(self, state, inp):
        ...
        print 'Front1,Front2:',round(front1,2),round(front2,2),'fleft,fright:',round(fleft,2),round(fright,2),'left,right:',round(left,2),round(right,2)   
  ...
  #              return state,z
            elif front2>0.8 and fright<1.9 and fleft<1.9:       #wall following
        ...
        if state[0]==2:
            ...
            #swap destination and source
                                                           #########################
            if state[1]<=62:   # when low battery          #Test THIS!!!!!!!!!!!!!!#
        ...
        if state[0]==5:
            ...
            if state[1]<=9:                                          #Could be decreased when fully charged!!!!!
        ...
        if state[0]==13: # left3
            ...
            if round(right,2)>1.2 and round(front1,2)>1 or round(left,2)>1.2 and round(front1,2)>1: # a new junction
                ...
                state[1]=0
                return state, z
            elif state[1]>10 and fright>0.6:
        ...
        if state[0]==15: # left5
            if round(right,2)>1.2 and round(front1,2)>1 or round(left,2)>1.2 and round(front1,2)>1: # a new junction
                ...
                state[1]=0
                return state, z
            elif state[1]<=11: #last value 10，8
                z=io.Action(fvel=0.4,rvel=0)
                state[1]+=1
                return state, z
            else:
